...
| `MotorReposicion` | `dashmotorreposicion.exe` | 3019 | *(pendiente de alta)* | `server.js` | Tablero de reposición/quiebre de stock para Cegid, conexión directa a `db_Cegid` (10.0.0.115). Precálculo nocturno vía SQL Agent Job (06:30) sobre tablas `MotorReposicion_*`. Instalado como servicio el 01/09/2026; se agregó bind explícito a `HOST=127.0.0.1` en `server.js` (antes escuchaba en todas las interfaces, sin `HOST` en el `app.listen`). Falta alta en Portal → Administración → Dashboards y otorgar permisos. Ver `CLAUDE.md` propio.

...
| `tablero-objetivos-web` | `dashtableroobjetivos.exe` | 3012 | *(pendiente de alta)* | `server\server.js` | Objetivos Sucursal — port web de `TABLERO OBJETIVO SUCURSALES OLD.qvw` (hoja SH23). Reusa el puerto 3012 liberado por ControlAcceso. **Conectado a SQL Server real** (10.0.0.115, base **dw_vallejo** — no "TABLEROS"; sa/ver `.env`, sin TLS). Falta validar los números contra el `.qvw` y dar de alta en Portal → Administración → Dashboards. Ver `DEPLOY.md`, `README.md` y `server/consultas.js` (mapeo real + aproximaciones marcadas). UI (25/08/2026): se sacó la fila de pestaña "Objetivos Sucursal" (no aportaba, restaba pantalla) y se forzó modo claro siempre (se quitó el soporte de `prefers-color-scheme`/`data-theme` oscuro) en `server/public/index.html`. |
| `StockProveedorMarca` | `dashstockproveedormarca.exe` | 3017 | `/d/19/` | `server\server.js` | Stock sumado por proveedor y marca, por mes/año y empresa (Tesi/Pueblo/Todas), desde `db_Cegid.dbo.FotoStockMES2` (56 fotos de fin de mes, desde 2021-12-31). Sólo lectura, cachea por (fecha, empresa) porque los meses cerrados no cambian. Ver `CLAUDE.md`. |
| `VentaObjetivo` | `dashventaobjetivo.exe` | 3016 | `/d/18/` | `server\server.js` | Ventas Comparativas — port web de `VENTASCOMPARATIVAS.qvw`: 5 pivots Año×Mes (Ventas/Operaciones/Unidades/Tkt Prom/Uni x Cli) desde `TABLEROS.dbo.GrillaVentasComparativas` (meses cerrados) + `dw_vallejo.f_objetivos` bridgeado vía `l_sucursal` para el mes en curso (resaltado, sin cerrar). Además, **carga de objetivos del mes siguiente**: columna editable inline por sucursal (Uni x Cli/Tkt Prom/Operaciones → Unidades/Venta $ calculados), borrador local en `data-store/` hasta guardar (INSERT/UPDATE transaccional a `f_objetivos`, por empresa o red completa). "Calzados 01" no carga objetivo (deshabilitada a pedido); "Calz SJ LIQUIDACION" excluida de todo el tablero. Ver `CLAUDE.md`, `server/consultas.js` y `extraido-del-qvw/` (layout recuperado del `.qvw`, formato binario propietario). |
| `VentaObjetivoSucursal` | `dashventaobjetivosucursal.exe` | 3018 | `/d/20/` | `server\server.js` | Clon 100% solo lectura de VentaObjetivo, para encargados de UNA sola sucursal. La sucursal se resuelve dinámicamente por usuario del portal contra `TABLEROS.dbo.EncargadosSucursalObjetivos` (columna `Usuario`, sólo si tiene exactamente 1 fila — `HAVING COUNT(*)=1`); sin match, no ve nada (fail-closed). Sidebar de Filtros/Objetivos oculto y botones de grupo (Pueblo/Tesi/Digitales) recortados a la empresa real de esa sucursal. 40 usuarios dados de alta (permiso sólo a este dashboard) el 31/08/2026. Histórico limitado a 2 años atrás (Comparativas y Totales). Ver `CLAUDE.md`. |
| `MotorReposicion` | `dashmotorreposicion.exe` | 3019 | `/d/21/` | `server.js` | Tablero de reposición/quiebre de stock para Cegid, conexión directa a `db_Cegid` (10.0.0.115). Precálculo nocturno vía SQL Agent Job (06:30) sobre tablas `MotorReposicion_*`. Instalado como servicio el 01/09/2026; se agregó bind explícito a `HOST=127.0.0.1` en `server.js` (antes escuchaba en todas las interfaces, sin `HOST` en el `app.listen`). Dado de alta en el portal (Dashboard Id **21**) y con permisos otorgados el 01/09/2026. Ver `CLAUDE.md` propio.

...
- `sp_ReporteVentasHistorico` (endpoint `/api/reporte-ventas-historico`, sobre `dw_vallejo.f_vta_cabecera`) también expone desde 2026-07-24 `DNI_CLIENTE`/`APE_CLIENTE`/`NOM_CLIENTE`, cruzando por sucursal+ticket contra `CGD_CONDCOM_OPERACION` (solo ventas con promo) y `Vta_cab_HORA` (fallback de nombre para el resto), más un tercer fallback (mismo día, 2026-07-24) para ventas WEB/MercadoLibre que cruza solo por `NRO`+fecha (sin sucursal) contra `Vta_cab_HORA` con `ORIGEN='ECO'`, ya que esas ventas no tienen SUC real en `db_Cegid`. Ver detalle de los joins en `CONTEXT.md`.

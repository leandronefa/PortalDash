...
/* VentaObjetivoSucursal (31/08/2026): sucursal única del usuario del portal
   logueado, para el tablero de solo-lectura de un solo encargado. A
   diferencia de SUCURSALES_DE_ENCARGADO (filtra por NombreApellido, usado
   por VentaObjetivo con un mapa hardcodeado usuario→encargado), esto filtra
   directo por la columna Usuario (case-insensitive) y sólo resuelve algo si
   ese Usuario tiene EXACTAMENTE una fila en la tabla — encargados de más de
   una sucursal quedan afuera (fail-closed, sin match = 0 filas = no ve
   nada), porque ellos ya tienen acceso al VentaObjetivo original. */
const SUCURSAL_UNICA_DE_USUARIO = `
  WITH raw AS (
    SELECT Usuario, Sucursal AS Codsuc
    FROM TABLEROS.dbo.EncargadosSucursalObjetivos
    WHERE UPPER(Usuario) = UPPER(@usuario)
      AND UPPER(Usuario) IN (

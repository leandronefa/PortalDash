...
const {
  ...
  OBJETIVOS_DEL_MES, DIAS_MARGEN_DEL_MES, MESES_CON_OBJETIVO, DELETE_OBJETIVO, SUPERVISORES, SUCURSAL_UNICA_DE_USUARIO,
...
async function obtenerMesesConObjetivoCacheados() {
  ...
  // Filtra basura de pruebas viejas, y respeta el límite de histórico de
  // este tablero (2 años atrás) además del tope del mes editable.
  const tope = mesObjetivoAAAAMM();
  const piso = mesLimiteHistorico();
  const data = r.recordset.map((row) => row.id_mes).filter((m) => m >= piso && m <= tope);
...
/* "Totales" para cualquier mes que NO sea el editable (31/08/2026, ajustado
   30/08/2026, portado de VentaObjetivo) — SIEMPRE muestra el objetivo
   cargado para ese mes (no la venta real: eso es cosa de Comparativas).
   Sale 100% de dw_vallejo (f_objetivos + f_dias_habiles), nunca de
   GrillaVentasComparativas — así un mes recién cerrado (sin fila real
   todavía por el ETL) también se puede ver, y un mes viejo muestra lo que
   se había targeteado, no lo que terminó pasando. 100% solo lectura como
   el resto de este tablero (no hay borrador, no hay ajuste editable, no
   hay guardado — acá nunca, a diferencia de VentaObjetivo, que sí admite
   ajuste puntual del mes en curso para vallejo/admin: acá no hay ese
   usuario). El filtro por sucursal única (sucursalesPermitidas) se aplica
   después, igual que en el resto de los endpoints. */

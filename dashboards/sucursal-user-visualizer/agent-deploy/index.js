...
const PASSRESET_EXCLUDE_USERS = new Set(
  (process.env.PASSRESET_EXCLUDE_USERS || 'Administrador,Administrator,SYSTEM,DefaultAccount,WDAGUtilityAccount,Invitado,Guest')
...
async function passresetRegistrarUsuarios(db) {
  ...
  for (const usuario of localUsers) {
    ...
    if (PASSRESET_EXCLUDE_USERS.has(usuario.toLowerCase())) continue;
    try {
      await db.request()

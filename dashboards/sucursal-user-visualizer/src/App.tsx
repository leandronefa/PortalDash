...
export default function App() {
  ...
  const loadData = useCallback((isInitial = false) => {
    ...
    fetchData()
      .then(rows => {
        setRawRows((prev: DataRow[]) => {
          // Si la cantidad de filas y el contenido de la primera y última son iguales,
          // conservamos la misma referencia para que el grafo NO se reconstruya.
          if (
            prev.length === rows.length &&
            prev.length > 0 &&
            JSON.stringify(prev[0]) === JSON.stringify(rows[0]) &&
            JSON.stringify(prev[prev.length - 1]) === JSON.stringify(rows[rows.length - 1])
          ) {
            return prev;
          }
          return rows;
  ...
  const toggleFullscreen = useCallback(() => {
    ...
    if (!document.fullscreenElement) {
      graphRef.current.requestFullscreen().catch(() => {});
    } else {
      ...
      <div className="flex-1 flex gap-8 relative overflow-hidden">
        ...
        <main ref={graphRef} className="flex-[2] relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden flex flex-col justify-center items-center transition-colors">
          <div className="absolute top-6 left-6 z-10 flex flex-col gap-2">
            <ServerStatusPanel isDark={isDark} />
          </div>
          
          <div className="absolute top-6 right-6 z-10 flex flex-col items-end gap-2">
            {/* Leyenda compacta */}
            <div className="bg-white/90 dark:bg-slate-900/90 px-3 py-1.5 rounded-xl shadow-lg backdrop-blur-sm border border-slate-100 dark:border-slate-700 flex items-center gap-3">
              <span className="flex items-center gap-1 text-[10px] text-slate-500 dark:text-slate-400">
                <span className="w-2 h-2 rounded-full bg-blue-500 shrink-0" />Sucursal
              </span>
              <span className="flex items-center gap-1 text-[10px] text-slate-500 dark:text-slate-400">
                <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" />Usuario
              </span>
              <span className="flex items-center gap-1 text-[10px] text-slate-500 dark:text-slate-400">
                <span className="w-2 h-2 rounded-full bg-purple-500 shrink-0" />Supervisor
              </span>
            </div>
            {/* Botones de vista */}
            <div className="bg-white/90 dark:bg-slate-900/90 p-1.5 rounded-xl shadow-lg backdrop-blur-sm border border-slate-100 dark:border-slate-700 flex items-center gap-1 transition-colors">
            <button

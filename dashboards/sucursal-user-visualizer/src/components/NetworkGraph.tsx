...
export default function NetworkGraph({ nodes, links, selectedNode, connectedNodes, layoutType, isDark }: NetworkGraphProps) {
  ...
  const validNodesRef = useRef<any[]>([]);
  ...
  useEffect(() => {
    ...
    // Calcular huella de la topología — si es igual, no reconstruir el grafo
    ...
    const topoKey = `${layoutType}::${isDark}::${resetKey}::${nodeKey}::${linkKey}`;
    if (topoKey === topoKeyRef.current) return;
    ...
    // Store validNodes in ref for use in selection effect
    validNodesRef.current = validNodes as any[];
    ...
    const drag = d3.drag<SVGCircleElement, any>()
      ...
      .on('end', (event, d) => {
        ...
        // No click selection from the graph: drag only.
        // Keep d.fx and d.fy as they are so the node stays anchored.
    ...
    // Handle resize — window resize + fullscreen change
    const handleResize = () => {
      if (!containerRef.current || !svgRef.current) return;
